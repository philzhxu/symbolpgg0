...
class Constants(BaseConstants):
    name_in_url = 'round_appropriation_full'
    players_per_group = 5
    num_rounds = 20
    ...
    cost_select_B = 1
    entrance_fee = 5
    Rule_base = 'rule_appropriation_full/Rule_base.html'
...
class Group(BaseGroup):
    ...
    game = models.StringField(initial="Dickson appropriation full")
    ...
    def set_deposit(self):
        ...
        for player in players:
            player.round_deposit = self.public2deposit + (
                    player.in_round(self.round_number).is_selfish in (None, True)) * Constants.base_deposit
        A = self.get_player_by_id(self.A_id)
        A.round_deposit = self.public2deposit
    ...
    def set_B_vote(self):
        ...
        if self.A_continue == 0:
            ...
            for player in players:
                player.real_round_deposit = player.round_deposit - bool(player.B_vote) * Constants.cost_select_B - (
                        player.id_in_round == self.A_continue and self.is_punish) * 30
            A = self.get_player_by_id(self.A_id)
            A.real_round_deposit = A.round_deposit + self.is_punish*Constants.base_punish - bool(self.A_continue) * Constants.cost_select_A
