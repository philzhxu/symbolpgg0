...
class Constants(BaseConstants):
    ...
    cost_select_B = 1
    entrance_fee = 5
    Rule_base = 'rule_base/Rule_base.html'
...
class Group(BaseGroup):
    ...
    #
    game = models.StringField(initial="Dickson salary full")
    deposit_sum = models.IntegerField(initial=0)
    public2deposit = models.FloatField()
    A_continue = models.IntegerField(choices=[(0,'不选择任何人'),*[(i,'ID为%d的参与者'%i) for i in range(1,Constants.num_B+1)]],label='',widget=widgets.RadioSelectHorizontal)
    ...
    def set_deposit(self):
        ...
        for player in self.get_players():
            player.round_deposit = self.public2deposit+(player.in_round(self.round_number).is_selfish in (None,True))*20
    ...
    def set_B_vote(self):
        ...
        if self.A_continue == 0:
            ...
            for player in players:
                player.real_round_deposit = player.round_deposit - bool(player.B_vote)*1 - (player.id_in_round == self.A_continue and self.is_punish)*30
    ...
    def get_sorted_id_in_round_contain_A(self):
        players = [i for i in self.get_players() if i.id_in_group != self.A_id]
        players.sort(key=lambda x:x.id_in_round)
        return [("角色A","未参与",self.get_player_by_id(self.A_id).real_round_deposit)]+[(i.id_in_round,'都留在私人账户' if i.is_selfish else '都存入本组公共账户',i.real_round_deposit) for i in players]
...
class Player(BasePlayer):
    ...
    is_selfish = models.BooleanField(label='',choices=[(True,'都留在私人账户'),(False,'都存入本组公共账户')],widget=widgets.RadioSelectHorizontal)
    round_deposit = models.FloatField()
    B_vote = models.IntegerField(choices=[(0,'什么也不做'),(1,'同意A的决策'),(-1,'反对A的决策')],widget=widgets.RadioSelectHorizontal,label='')
