...
class Constants(BaseConstants):
    name_in_url = 'round_appropriation_limited'
    players_per_group = 5
    num_rounds = 20
...
class Group(BaseGroup):
    ...
    game = models.StringField(initial="Dickson appropriation limited")
    ...
    def set_B_vote(self):
        ...
        if self.A_continue == 0:
            ...
            A = self.get_player_by_id(self.A_id)
            A.real_round_deposit = A.round_deposit + self.is_punish * Constants.base_punish - bool(
                self.A_continue) * Constants.cost_select_A
